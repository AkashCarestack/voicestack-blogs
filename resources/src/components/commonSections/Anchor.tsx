...
import { getCssSelectorShort } from '~/resources/helpers/createCSSSelector'
import { getParams, getQueryParamFromLink } from '~/resources/helpers/getQueryParams'
import { generateId, getResourcesCmsLocale, resolveResourcesHref, withRegionalResourcesLocale } from '~/resources/utils/common'
import { getCookie } from '~/resources/utils/tracker/cookie'
import { useTrackUser } from '~/resources/utils/tracker/intitialize'
...
interface LinkProps {
  ...
  isDemo?: boolean
  /** Skip auto-prefixing /resources paths with the current regional locale (e.g. region switcher). */
  disableRegionalPrefix?: boolean
  [x: string]: any
  className?: string
}
...
const Anchor: React.FunctionComponent<LinkProps> = ({
  ...
  target,
  className,
  disableRegionalPrefix = false,
  ...rest
}) => {
  const router = useRouter();
  const cmsLocale = getResourcesCmsLocale(router)
  const baseHref = resolveResourcesHref(href)
  const resolvedHref = disableRegionalPrefix
    ? baseHref
    : withRegionalResourcesLocale(baseHref, cmsLocale)
  ...
  useEffect(() => {
    if (typeof resolvedHref !== 'string' || !resolvedHref) {
      setNewLink('#');
      return;
    }
    ...
    const existingParams = resolvedHref.includes('?') ? resolvedHref.split('?')[1] : '';
    ...
    if (router.asPath.startsWith("/lp")) {
      ...
      setNewLink(`${resolvedHref.split('?')[0]}${updatedParams.length > 0 ? "?" + updatedParams : ""}`);
    }
  }, [resolvedHref, router, trackCtx]);


    return (
      <Link id={generateId(resolvedHref)}
      onClick={(e) => {
        ...
        trackEvent({
          ...
          e_time: new Date(),
          element,
          element_id: generateId(resolvedHref),
