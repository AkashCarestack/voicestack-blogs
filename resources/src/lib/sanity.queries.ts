...
import { Author, Post, Tag } from '~/resources/interfaces/post'
...
`
// sitemap queries **
export const siteMapQuery = groq`*[_type == "post" && !(_id in path("drafts.**")) && defined(slug.current)]{
  "url": slug.current,
  contentType,
  _updatedAt,
  language
...
export const tagQuery = groq`*[_type == "tag" && !(_id in path("drafts.**")) && defined(slug.current)]{
  "url": slug.current,
  "contentType": "browse",
  _updatedAt
} `

export const categorySitemapQuery = groq`*[_type == "category" && !(_id in path("drafts.**")) && defined(slug.current)]{
  "url": slug.current,
  "contentType": "topic",
  _updatedAt
} `

export const authorQuery = groq`*[_type == "author" && !(_id in path("drafts.**")) && defined(slug.current)]{
  "url": slug.current,
  "contentType": "author",
  _updatedAt
} `
...
*[_type == "testimonial" && !(_id in path("drafts.**")) && defined(slug.current)]{
  "url": slug.current,
...
export async function getSitemapData(client: SanityClient): Promise<SitemapRow[]> {
  try {
    const [posts, tags, categories, authors, testimonials] = await Promise.all([
      client.fetch(siteMapQuery),
      client.fetch(tagQuery),
      client.fetch(categorySitemapQuery),
      client.fetch(authorQuery),
      client.fetch(testiMonialQuery),
    ])
    return [...posts, ...tags, ...categories, ...authors, ...testimonials]
  } catch (error) {
    console.error(error)
    return []
  }
