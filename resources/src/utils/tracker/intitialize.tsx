...
import { getParams } from "~/resources/helpers/getQueryParams";
  ...
  if (!getSession() && usr !== null) {
    ...
    try {
      if (typeof _session === "object") {
        const res = await fetch("/api/resources/track?type=session", {
          method: "POST",
...
export const createUser = async (deviceData: any) => {
  try {
    ...
    if (_user === null) {
      ...
      const res = await fetch("/api/resources/track?type=user", {
        method: "POST",
...
export async function getUserData(id: string) {
  try {
    const res = await fetch("/api/resources/track?type=user&id=" + id);
...
export function TrackUserProvider(props: PropsWithChildren) {
  ...
  //           if (userId !== null) {
  //             await fetch("/api/resources/track?type=session&" + `id=${res?.sessionId}`, {
  //               method: "PATCH",
  //               headers: { "Content-Type": "application/json" },
  //               body: JSON.stringify({
  //                 id: userId,
...
export const createObservedUser = (data: any) => createObservableObject(data, async (prop, val) => {
  const userId = getUser();
  if (userId !== null) {
    await fetch("/api/resources/track?type=user", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        id: userId,
