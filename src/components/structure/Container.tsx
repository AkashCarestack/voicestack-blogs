...
interface ContainerProps {
  ...
  className?: string
  type?: "V1" | "V2"
  border?: "all" | "t-0" | "b-0" | "y-0"
  innerPadding?: boolean
}

export default function Container(props:ContainerProps) {
  const { children, className, type = "V1", border = "none", innerPadding = false } = props
  const borderClass = border === "all" ? "border border-gray-200" : border === "t-0" ? "border border-gray-200 border-t-0" : border === "b-0" ? "border border-gray-200 border-b-0" : border === "y-0" ? "border border-gray-200 border-y-0" : ""
  if (type === "V2") {
    return (
