...
async function getPageDocumentDates(
  client: any,
  paths: Set<string>,
): Promise<Map<string, PathEntry>> {
  const allPages = await getAllPageDocuments(client)
  const result = new Map<string, PathEntry>()

  paths.forEach((path) => {
    const pageData = allPages.get(path)
    if (pageData) {
      result.set(path, pageData)
    }
  })
...
export async function buildVoiceStackSitemapXml(client: any): Promise<string> {
  ...
  pageDates.forEach((pageData, path) => {
    ...
    if (existing) {
      ...
      existing.locales = Array.from(combinedLocales)
      existing.date = mergeContentDate(existing.date, pageData.date)
    } else {
      navigationPaths.set(path, {
        locales: pageData.locales,
        ...(pageData.date && { date: pageData.date }),
      })
    }
